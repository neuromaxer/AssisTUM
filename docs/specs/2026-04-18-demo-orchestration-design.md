# AssisTUM Demo Orchestration Design

**Goal:** Engineer the agent prompts, skills, and demo script so that a 5-minute fully-live demo reliably produces a memorable "one prompt → full calendar" moment, followed by interactive follow-ups that show intelligence and personality.

**Approach:** "Narrated Cascade" — one user message triggers a multi-phase planning sequence. The agent narrates progress between tool calls so the audience sees events/todos appearing in the UI while the chat explains what's happening. After the cascade, the presenter asks 1-2 follow-up questions to demonstrate interactivity.

**Demo structure:**
- **Act 1 (~2.5 min):** "Let's plan my next week" → narrated cascade across all data sources
- **Act 2 (~1 min):** Presenter adds personal events via natural language
- **Act 3 (~1 min):** "Schedule study sessions before my deadlines" → agent reasons about free time
- **Bonus (if time):** Commute helper or canteen question

---

## 1. System Prompt (opencode.json instructions)

Replace the current 4-line instruction array with a comprehensive system prompt that establishes personality, tool-calling discipline, narration behavior, and failure handling.

```
You are AssisTUM, a campus co-pilot for TUM students. You speak concisely
and warmly — like a knowledgeable friend, not a corporate assistant.
Use short paragraphs, not bullet lists. Address the user by name if known.

CRITICAL BEHAVIOR — TOOL CALLS:
- Every event, todo, and course MUST be created via tool calls (create_event,
  create_todo, create_course). These persist to the database and appear in
  the UI in real-time. NEVER just describe what you "would" add — actually
  call the tool.
- When creating events, always use ISO 8601 datetimes for the UPCOMING week
  (Monday to Sunday). Use the current date to calculate this.
- Color coding: lectures #3070b3, study #22c55e, meals #f97316, clubs #a855f7,
  recreation #ec4899, custom #6b7280.

CRITICAL BEHAVIOR — NARRATION:
- When doing multi-step work, narrate your progress in SHORT status lines
  between tool calls. Example: "Pulling your lectures from TUM Online..."
  then call tools, then "Found 4 lectures — added them to your calendar."
- Never go silent for more than 2-3 tool calls. Always surface progress.
- After completing a batch of work, give a brief natural-language summary
  of what changed.

FAILURE HANDLING:
- If a fetch tool returns an error or "not configured", skip it gracefully.
  Say something like "Couldn't reach Moodle right now — I'll work with
  what I have" and continue with other sources.
- Never stop the entire workflow because one source failed.

PERSONALITY:
- Be proactive: suggest lunch between back-to-back lectures, warn about
  early mornings, notice long gaps that could be study time.
- Show you understand student life — mention things like "that's a tight
  turnaround" or "good news, you've got Wednesday afternoon free."
```

### Design rationale

- **"Always call tools"** prevents the most common failure mode: the agent describing what it would do instead of doing it.
- **Narration requirement** ensures the audience always has something to watch in the chat panel while events appear on the calendar.
- **Per-phase failure handling** means one broken API (e.g., Moodle) doesn't derail the entire demo.
- **Warm personality** makes the demo feel like a product, not a tech proof-of-concept.

---

## 2. Plan-Week Skill (Act 1: The Cascade)

Replace the current thin 8-step checklist with a heavily scripted 6-phase skill that prescribes exact sequencing, narration beats, and per-phase degradation.

```markdown
---
name: plan-week
description: Plan the student's upcoming week by fetching all data
  sources and populating the calendar and todo list
---
...
- **Lectures first** because they produce the most dramatic visual change (multiple blue blocks appearing on the calendar).
- **Each phase is independent** so a failure in Phase 2 doesn't prevent Phase 4.
- **Narration is prescriptive** ("Tell the user: ...") rather than suggestive so the model follows it reliably.
- **The final question** naturally invites Act 2/3 of the demo.

---

## 3. Follow-Up Skills (Act 2 & 3)

### Schedule Study Sessions

The current skill asks the user about preferences — too slow for a demo. The rewrite makes the agent opinionated: it proposes a plan immediately and only asks for confirmation.

```markdown
---
name: schedule-study-sessions
description: Schedule study blocks based on assignment deadlines
...
Stays lightweight but with better narration:
...
- **Study sessions skill** is opinionated by design — proposing first, asking second. In a demo, watching the agent ask "what are your preferred study hours?" is dead time.
- **Conflict resolver** is conversational — it presents the problem and waits for a choice, which makes the demo interactive.
- **Commute helper** demonstrates live data (real MVV departures) in a single impressive answer.

---

## 4. Demo Script (presenter cheat-sheet)

Meant to be open on a phone or second screen during the demo.
...
Clear the DB for a clean re-run:
```bash
curl -X DELETE http://localhost:3001/api/reset
```
(Bulk reset endpoint that clears events, todos, and courses)

Alternatively, restart the backend with a fresh DB.
...
- After you say "looks good" (or just wait), green study blocks appear on the calendar
...
**If all APIs fail:**
Same fallback. Have a pre-seeded DB backup ready:
```bash
...
Things that need to be built or changed to support this design:

### Must-have

1. **Update `opencode.json` instructions** — replace the 4-line array with the full system prompt from Section 1.
2. **Rewrite `plan-week.md` skill** — replace with the 6-phase scripted version from Section 2.
3. **Rewrite `schedule-study-sessions.md`** — opinionated version from Section 3.
4. **Update `conflict-resolver.md`** — narration improvements from Section 3.
5. **Update `commute-helper.md`** — cleaner version from Section 3.
6. **Add bulk reset endpoint** — `DELETE /api/reset` that clears events, todos, and courses tables so the presenter can re-run the demo cleanly.
7. **Create `demo_script.md`** — the presenter cheat-sheet from Section 4, placed at `docs/demo_script.md`.

### Nice-to-have

8. **Pre-seeded backup DB** — a `scripts/seed-demo.sh` that populates realistic events/todos via curl as a fallback if the agent can't run live.
9. **Timing test** — run the full plan-week cascade once and measure end-to-end latency to calibrate presenter expectations.
